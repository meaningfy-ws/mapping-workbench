...
from mapping_workbench.backend.core.models.crud_routes import CRUDRoutes, CRUDFileResourceRoutes
...
class CRUDApiFileResourceRouter(Generic[T]):
    model: type[T]
    ...
    def init_router(self, entity_routes: CRUDFileResourceRoutes = None) -> APIRouter:
        router = APIRouter(
            prefix=self.meta.route_prefix,
            tags=self.meta.route_tags
        )

        routes: CRUDFileResourceRoutes = entity_routes or CRUDFileResourceRoutes()

        routes.has_route_get_all and self.add_api_route_get_all(router)
        routes.has_route_create_one and self.add_api_route_create_one(router)
        routes.has_route_create_many and self.add_api_route_create_many(router)
        ...
        routes.has_route_delete_many and self.add_api_route_delete_many(router)
