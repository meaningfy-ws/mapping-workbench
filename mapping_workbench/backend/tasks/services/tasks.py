...
async def terms_validator(filters: APIRequestWithProject = None):
    query_filters: Dict = {}
...
async def transform_test_data(filters: APIRequestWithProject = None):
    await transform_test_data_for_project(project_id=filters.project)


async def generate_cm_assertions_queries(filters: APIRequestWithProject = None):
    if filters.cleanup:
