        ...
        for issue in name[0][0]['validation_results']:
            if issue['result'] in ['warning', 'error', 'unverifiable', 'invalid']:
                if issueHash.get(issue['result']) == None:
                    issueHash.setdefault(issue['result'], []).append((str(name[1]) + "*" + str(issue['identifier']), issue['query']['title'], issue['query']['xpath'], issue['query']['query']))
                else:
                    issueHash[issue['result']].append((str(name[1]) + "*" + str(issue['identifier']), issue['query']['title'], issue['query']['xpath'], issue['query']['query']))
            listError.append((str(issue['identifier']), issue['query']['title'], issue['query']['xpath'], issue['query']['query']))
        ...
        for k in helperHash:
            for l in listError:
                if k == l[0]:
                    ...
                    secondHash[l[0]] = {"notices": helperHash[k], "notice_count": len(helperHash[k]), "details": {"title": l[1], "query": l[3], "xpath": l[2], "issue_code": "issue_" + hash_object.hexdigest()}}
    ...
    for name in data:
        for error in data[name]:
            for o in data[name][error]:
                for sparql in o:
                    if sparql != "count":
                        listProblem.append((error, o[sparql]['details']['issue_code'], name, o[sparql]['details']['title'], o[sparql]['details']['xpath'], o[sparql]['details']['query']))
                    ...
                    for notice_path in notices_output_path.iterdir():
                        if notice_path.is_dir():
                            ...
                            if sparql_validation_path.exists() and sparql_validation_path.is_file():
                                ...
                                packages_list_of_errors.append((sparql_validation_content, str(id).split("/output/")[1]))
