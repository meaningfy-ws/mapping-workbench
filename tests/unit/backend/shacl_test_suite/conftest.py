...
def shacl_test_data_file_path() -> pathlib.Path:
    return TEST_DATA_PATH / "shacl_test_suite" / "shacl_test_data" / "notice.ttl"


@pytest.fixture
def shacl_test_shape_file_path() -> pathlib.Path:
    return TEST_DATA_PATH / "shacl_test_suite" / "shacl_test_shape" / "ePO_shacl_shapes.xml"
...
@pytest.fixture
def dummy_shacl_test_suite(shacl_test_shape_file_path: pathlib.Path) -> SHACLTestSuite:
    return SHACLTestSuite(
        file_resources=[SHACLTestFileResource(content=shacl_test_shape_file_path.read_text(encoding="utf-8"),
                                              filename=shacl_test_shape_file_path.name,
