...
from mapping_workbench.backend.sparql_test_suite.entrypoints.api.routes import ROUTE_PREFIX
from tests.e2e.backend.api import client, api_endpoint


@pytest.mark.asyncio
async def test_sparql_test_suites_assign_mapping_packages_route(
        req_headers,
        dummy_project,
    ...
    assert len(mapping_package.sparql_test_suites) == 2
